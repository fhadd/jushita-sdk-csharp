...
namespace Top.Api.Request
{
    ...
    public class FenxiaoOrdersGetRequest : ITopRequest<FenxiaoOrdersGetResponse>
    {
        ...
        /// <summary>
        /// 交易状态，不传默认查询所有采购单。根据身份选择自身状态。可选值:<br>     *供应商：<br>  WAIT_SELLER_SEND_GOODS(等待发货)<br>  WAIT_SELLER_CONFIRM_PAY(待确认收款)<br>  WAIT_BUYER_PAY(等待付款)<br>  WAIT_BUYER_CONFIRM_GOODS(已发货)<br>  TRADE_REFUNDING(退款中)<br>  TRADE_FINISHED(采购成功)<br>  TRADE_CLOSED(已关闭)。<br>  WAIT_SELLER_SEND_GOODS_WAIT_ACOUNT (已付款（待分账），待发货。只对代销分账支持)<br>  WAIT_SELLER_SEND_GOODS_ACOUNTED(已付款（已分账），待发货。只对代销分账支持)<br>      *分销商：<br>  WAIT_BUYER_PAY(等待付款)<br>  WAIT_BUYER_CONFIRM_GOODS(待收货确认)<br>  TRADE_FOR_PAY(已付款)<br>  TRADE_REFUNDING(退款中)<br>  TRADE_FINISHED(采购成功)<br>  TRADE_CLOSED(已关闭)。<br>  WAIT_BUYER_CONFIRM_GOODS_WAIT_ACOUNT(已付款（待分账），已发货。只对代销分账支持)<br>  WAIT_BUYER_CONFIRM_GOODS_ACOUNTED(已付款（已分账），已发货。只对代销分账支持)<br>
        /// </summary>
